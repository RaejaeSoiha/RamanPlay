...
import { directMediaKind, directPlayableKind, directPlayableUrl } from "../../lib/playback";
...
type LinkForm = {
  ...
  playback_preference: "AUTO" | "DIRECT_MEDIA" | "EXTERNAL_PAGE";
};
...
function toForm(link: MyLink): LinkForm {
  return {
    ...
    playback_preference: link.playback_type === "DIRECT_MEDIA" ? "DIRECT_MEDIA" : "EXTERNAL_PAGE",
  };
}
...
export default function MyLinks({
  ...
  const [playingLinkId, setPlayingLinkId] = useState<number | null>(null);
  ...
  const formMediaKind = form ? directMediaKind(form.url) : null;
  const formIsDirect = Boolean(form && form.playback_preference !== "EXTERNAL_PAGE" && formMediaKind);
  ...
  async function save(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await request(
        editing === null ? (collectionPath || `/api/my-links/${gameId}`) : `/api/my-links/${editing}`,
        {
  ...
  async function startInlinePlayback(link: MyLink) {
    setBusy(true);
    setError("");
    try {
      const result = await request(`/api/my-links/${link.id}/opened`, { method: "POST" });
      setItems((current) => current.map((item) => (item.id === result.id ? { ...item, ...result } : item)));
      setPlayingLinkId(link.id);
    } catch (err) {
      setError((err as Error).message);
    } finally {
      setBusy(false);
  ...
  function openLink(link: MyLink | null) {
    if (directPlayableUrl(link)) {
      void startInlinePlayback(link!);
      return;
    }
  ...
  async function createWatchRoom() {
    if (!best || !csrfToken || !gameId) return;
  ...
  return (
    <section className="my-links">
      ...
      {opening === best?.id && best && (
        <div className="my-link-open-confirmation my-link-best-confirmation" role="status">
          {best.final_url && domainOf(best.final_url) ? (
            <>
              <span>
                {best.trust_state === "UNVERIFIED" ? "Unverified external link. " : "Opens in this tab: "}
                Destination: <strong>{best.final_destination_domain || domainOf(best.final_url)}</strong>
              </span>
              <button
                className="secondary-button"
                onClick={() => {
      ...
      {form && (
        <form className="my-link-form" onSubmit={save}>
          ...
          <label>
            Playback type
            <select
              value={form.playback_preference}
              onChange={(event) => setForm({ ...form, playback_preference: event.target.value as LinkForm["playback_preference"] })}
            >
              <option value="AUTO">Auto detect direct media</option>
              <option value="DIRECT_MEDIA">Direct media URL</option>
              <option value="EXTERNAL_PAGE">External webpage</option>
            </select>
          </label>
          ...
          <p>{formIsDirect ? "Direct stream — plays inside RamanPlay." : "External webpage — opens outside RamanPlay."}</p>
          <p>Public HTTPS links are allowed. Unverified external pages require confirmation before opening.</p>
          ...
          <article className="source my-link" key={link.id}>
            ...
            <div className="source-copy">
              <div className="source-title">
                ...
                <span className={`my-link-trust ${link.trust_state.toLowerCase()}`}>
                  {link.trust_state === "UNVERIFIED" ? "UNVERIFIED EXTERNAL LINK" : link.trust_state}
                </span>
                <span className="my-link-playback-type">
                  {link.playback_type === "DIRECT_MEDIA" ? "DIRECT STREAM" : link.playback_type.replaceAll("_", " ")}
                </span>
              ...
              {opening === link.id && link.id !== best?.id && (
                <div className="my-link-open-confirmation">
                  {link.final_url && domainOf(link.final_url) ? (
                    <>
                      <span>
                        {link.trust_state === "UNVERIFIED" ? "Unverified external link. " : "Opens in this tab: "}
                        Destination: <strong>{link.final_destination_domain || domainOf(link.final_url)}</strong>
                      </span>
                      <button
                        className="secondary-button"
                        onClick={() => {
            ...
            >
                {directPlayableUrl(link) ? <>Play here <Play size={16} /></> : <>Open Link <ExternalLink size={16} /></>}
              </button>
            ...
            {playingLinkId === link.id && directPlayableUrl(link) && <InlineVideoPlayer sourceUrl={directPlayableUrl(link)!} sourceType={directPlayableKind(link)!} title={link.source_name} isLive={live} onClose={() => setPlayingLinkId(null)} onOpenWatchOptions={() => setPlayingLinkId(null)} />}
          </article>
        ))}
      </div>
      {live && best && gameId && (
