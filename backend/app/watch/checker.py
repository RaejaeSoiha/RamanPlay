...
from app.watch.security import is_blocked_domain, validate_public_url
...
class LinkChecker:
    ...
    async def _check(self, url, max_redirects, warn_on_suspicious_redirects):
        ...
        try:
            for redirect in range(max_redirects + 1):
                ...
                validate_public_url(current, resolve=False)
            ...
            return (
                "BLOCKED" if warn_on_suspicious_redirects else "ERROR",
                code,
                current,
                time.monotonic() - started,
                f"Destination rejected ({exc})",
                redirect,
            )
    ...
    async def check_all(self, db):
        ...
        for link in personal_links:
            ...
            from app.watch.links import apply_personal_check
